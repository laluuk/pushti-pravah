...
class DataService {
    ...
    getKirtans(search = '', filter = 'All') {
        return this.kirtans.filter(k => {
            ...
            const kirtanSamays = Array.isArray(k.samay)
                ? k.samay
                : [k.samay];

            const matchesFilter =
                filter === 'All' ||
                kirtanSamays.includes(filter);
    ...
    getCurrentSamay() {
        ...
        const next =
            schedule.find(s => s.minutes > currentMinutes) ||
            schedule[0];
    ...
    getDynamicRecommendations() {
        ...
        let relevantKirtans = this.kirtans.filter(k => {
            ...
            return (
                kirtanSamays.includes(samayName) ||
                kirtanSamays.includes('Any Time')
            );
        });
        ...
        const relevantleelas = this.leelas;
    ...
    getKirtanTabs() {
        ...
        return [
            'All',
            ...this.config.appConfig.samaySchedule.map(s => s.name)
        ];
    }
...
class SettingsService {
    ...
    setLyricsFontSize(size) {
        this.settings.lyricsFontSize = size;
        this.save();
    }
    ...
    getLyricsFontSize() {
        return this.settings.lyricsFontSize || '1.5rem';
    }
}
...
class Router {
    constructor(dataService, settingsService) {
        ...
        // Apply saved lyrics font size
        document.documentElement.style.setProperty(
            '--lyrics-font-size',
            this.settingsService.getLyricsFontSize()
        );
    ...
    async navigate(view, pushState = true) {
        ...
        if (view === 'home') {
            ...
            const kirtan =
                await this.dataService.getKirtanById(id);
            ...
            const story =
                await this.dataService.getStoryById(id);
    ...
    updateNav() {
        ...
        const isTopLevel =
            topLevelViews.includes(this.currentView);

        const backBtn =
            document.getElementById('back-nav-btn');

        const logoIcon =
            document.getElementById('logo-icon-container');

        if (backBtn) {
            backBtn.style.display =
                isTopLevel ? 'none' : 'flex';
        }

        if (logoIcon) {
            logoIcon.style.display =
                isTopLevel ? 'flex' : 'none';
        }
        ...
        document.querySelectorAll('.nav-btn')
            .forEach(btn => {
                ...
                if (view === 'home') {
                    ...
                    isActive =
                        this.currentView === 'kirtans' ||
                        this.currentView.startsWith('kirtan-');
                }

                else if (view === 'leelas') {
                    isActive =
                        this.currentView === 'leelas' ||
                        this.currentView.startsWith('story-') ||
                        this.currentView.startsWith('story-list-');
                }
        ...
        if (topNavTitle) {
            if (this.currentView === 'kirtans') {
                ...
                this.dataService
                    ...
                    .then(() => {
                        const storyFromIndex =
                            this.dataService.leelas.find(
                                si => si.id === storyId
                            );

                        topNavTitle.textContent =
                            this.dataService.getLocalized(
                                storyFromIndex?.title
                            ) || 'Story';
                    });
            }
    ...
    renderHome() {
        const samay =
            this.dataService.getCurrentSamay();
        ...
        const recommendations =
            this.dataService.getDynamicRecommendations();
        ...
        this.mainContent.innerHTML = `
            <div class="home-view">
                ...
                <section class="mb-8">
                    <span class="nav-label text-secondary letter-spacing-wider">
                        UK TIME ${ukTime}
                    </span>

                    <h2 class="text-primary font-size-3xl margin-top-xs">
                ...
                <section class="mb-12">
                    <div class="samay-card samay-hero-card">
                        ...
                        <div class="samay-info">
                            ...
                            <div>
                                <span class="nav-label samay-hero-label">
                                    Next Darshan • ${samay?.time || ''}
                                </span>

                                <h3 class="samay-title text-white font-size-3xl">
                                    ${samay?.name || 'Loading...'}
                                </h3>
                            </div>
                ...
                <section class="mb-12">
                    ...
                    <div class="flex items-center justify-between mb-8">
                        ...
                        <div>
                            ...
                            <h3 class="text-primary font-size-3xl margin-top-xs">
                                ${samay?.name || 'Daily'} Kirtans
                            </h3>
                        </div>
                    ...
                    <div class="kirtan-list">
                        ...
                        ${recommendations.kirtans.map(k => {
                            ...
                            const samayDisplay =
                                Array.isArray(k.samay)
                                    ? k.samay.join(' • ')
                                    : k.samay;

                            return `
                                <div
                                    onclick="router.navigate('kirtan-${k.id}')"
                                    class="kirtan-item home-kirtan-item"
                                >

                                    <div class="kirtan-info">

                                        <span class="nav-label text-secondary margin-bottom-xs display-block opacity-70">
                                            ${k.poet} • ${samayDisplay}
                                        </span>

                                        <h4 class="margin-0 font-size-lg">
                                            ${title}
                                        </h4>
                    ...
                    <div class="mt-6">
                        ...
                        <button
                            onclick="router.setKirtanFilterAndNavigate('${recommendations.samay}')"
                            class="btn-secondary btn-home-cta"
                        >
                ...
                <section class="mb-12">
                    ...
                    <div class="gallery-grid">
                        ...
                        ${recommendations.leelas.map(s => `
                            ...
                            <div
                                onclick="router.navigate('story-${s.id}')"
                                class="tonal-card home-story-card"
                            >

                                <div class="flex items-center justify-between mb-6">

                                    <span class="nav-label text-secondary font-size-xs">
                                        ${s.category}
                                    </span>
                                ...
                                <h4 class="text-primary font-size-2xl margin-bottom-sm line-height-tight">
                                    ${this.dataService.getLocalized(s.title)}
                                </h4>
                                ...
                                <button class="btn-link-primary w-full padding-sm">
                                    Read Chronicle
                                </button>
                    ...
                    <div class="mt-6">
                        ...
                        <button
                            onclick="router.navigate('leelas')"
                            class="btn-secondary btn-home-cta"
                        >
                            Explore Granths
                        </button>
    ...
    renderKirtans() {
        const tabs =
            this.dataService.getKirtanTabs();
    ...
    renderKirtanListHtml() {
            ...
            return `
                <div class="tonal-card text-center padding-xl">
                    ...
                    <img
                        ...
                        alt="Empty"
                        class="icon-3xl opacity-10"
                    >

                    <p class="opacity-60 font-size-lg margin-top-sm">
        ...
        return kirtans.map(k => {
            ...
            const title =
                this.dataService.getLocalized(k.title);
            ...
            return `
                ...
                >
                    ...
                    <div class="kirtan-info">
                        ...
                        <span class="nav-label text-primary margin-bottom-xs display-block">
                            ${samayDisplay}
                        </span>
    ...
    renderleelas() {
        const categories =
            this.dataService.getStoryCategories();
        ...
        this.mainContent.innerHTML = `
            <div class="gallery-grid">
                ...
                ${categories.map(cat => `
                    ...
                    <div
                        onclick="router.navigate('story-list-${encodeURIComponent(cat.id)}')"
                        class="tonal-card story-category-card"
                    >
                        ...
                        <h4 class="text-primary font-size-3xl margin-bottom-sm">
                            ${cat.name}
                        </h4>

                        <p class="opacity-80 font-size-md margin-bottom-md">
                            ${cat.desc}
                        </p>
        ...
        if (!leelas.length) {
            return `
                <div class="tonal-card text-center padding-xl">
                    ...
                    <p class="opacity-60 font-size-lg">
            ...
            return `
                ...
                >
                    ...
                    <div class="kirtan-info">
                        ...
                        <p class="opacity-60 font-size-xs margin-top-xs">
                            ${s.excerpt}
                        </p>
        ...
        this.mainContent.innerHTML = `
            <div class="detail-container">
                ...
                <div class="detail-header mb-8">
                    ...
                    <h2 class="text-primary font-size-3xl text-center">
                        ${this.dataService.getLocalized(story.title)}
                    </h2>
                ...
                <div class="tonal-card padding-lg mb-8">
                    ...
                    <p class="font-size-lg story-content-text mb-6">
                        ${this.dataService.getLocalized(story.content)}
                    </p>
                    ...
                    <div class="kirtan-verses">
                        ...
                        ${[1, 2, 3].map(() => `
                            ...
                            <div class="verse-block text-center mb-16 relative">
                                ...
                                <div
                                    class="verse-number-display mb-4 skeleton-box"
                                    style="
                                        width: 40px;
                                        height: 40px;
                                        margin: 0 auto;
                                ...
                                <div
                                    class="detail-lyrics mb-4 skeleton-box"
                                    style="
                                        height: 2.5rem;
                                        width: 80%;
                                        margin: 0 auto;
    ...
    renderStorySkeleton() {
        this.mainContent.innerHTML = `
            <div class="detail-container">
                ...
                <div
                    class="detail-header mb-8 skeleton-box"
                    style="
                        height: 3rem;
                        width: 60%;
                        margin: 0 auto;
        ...
        this.mainContent.innerHTML = `
            <div class="detail-container">
                ...
                <div class="tonal-card mb-8">
                    ...
                    <h3 class="text-primary font-size-2xl mb-6">
                        Profile Settings
                    </h3>

                    <div class="mb-8">

                        <label class="nav-label text-secondary display-block mb-2">
                            Your Name
                        </label>
                ...
                <div class="tonal-card mb-8">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex gap-4 flex-wrap">
                            ...
                            <button
                                onclick="router.setLanguage('english')"
                                class="tab-btn ${language === 'english' ? 'active' : ''}"
                            >
                                English
                            </button>

                            <button
                                onclick="router.setLanguage('hindi')"
                                class="tab-btn ${language === 'hindi' ? 'active' : ''}"
                            >
                                Hindi
                            </button>

                            <button
                                onclick="router.setLanguage('gujarati')"
                                class="tab-btn ${language === 'gujarati' ? 'active' : ''}"
                            >
                                Gujarati
                            </button>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div>
                                ...
                                <label class="nav-label text-secondary display-block mb-1">
                                    Show Translations
                                </label>

                                <p class="opacity-60 font-size-sm">
                            ...
                            <button
                                onclick="router.toggleTranslation()"
                                class="btn-secondary ${showTranslation ? 'bg-accent' : ''}"
                            >
                        ...
                        ${showTranslation ? `
                            ...
                            <div class="mt-6">
                                ...
                                <div class="flex gap-4 flex-wrap">
                                    ...
                                    <button
                                        onclick="router.setTranslationLanguage('english')"
                                        class="tab-btn ${translationLanguage === 'english' ? 'active' : ''}"
                                    >
                                        English
                                    </button>

                                    <button
                                        onclick="router.setTranslationLanguage('hindi')"
                                        class="tab-btn ${translationLanguage === 'hindi' ? 'active' : ''}"
                                    >
                                        Hindi
                                    </button>

                                    <button
                                        onclick="router.setTranslationLanguage('gujarati')"
                                        class="tab-btn ${translationLanguage === 'gujarati' ? 'active' : ''}"
                                    >
                                        Gujarati
                                    </button>
                    ...
                    <div class="mb-8">
                        ...
                        <p class="opacity-60 font-size-sm mb-4">
                            Adjust the size of the Kirtan lyrics.
                        </p>

                        <div class="flex gap-4 flex-wrap">

                            <button
                                onclick="router.setLyricsFontSize('1.25rem')"
                                class="tab-btn ${lyricsFontSize === '1.25rem' ? 'active' : ''}"
                            >
                                Small
                            </button>

                            <button
                                onclick="router.setLyricsFontSize('1.5rem')"
                                class="tab-btn ${lyricsFontSize === '1.5rem' ? 'active' : ''}"
                            >
                                Medium
                            </button>

                            <button
                                onclick="router.setLyricsFontSize('1.75rem')"
                                class="tab-btn ${lyricsFontSize === '1.75rem' ? 'active' : ''}"
                            >
                                Large
                            </button>

                            <button
                                onclick="router.setLyricsFontSize('2rem')"
                                class="tab-btn ${lyricsFontSize === '2rem' ? 'active' : ''}"
                            >
                                Extra Large
                            </button>
    ...
    toggleTranslation() {
        ...
        this.settingsService.setTranslationToggle(
            !current
        );
    ...
    setLyricsFontSize(size) {
        this.settingsService.setLyricsFontSize(size);
...
const dataService =
    new DataService(settingsService);
